...
const providerSchema = joi.object({
	...
	addBuilding: joi.string().trim().required().min(2).max(100).messages({
		'string.empty': 'Building Name can not be empty.',
	}),
	addL1: joi.string().trim().required().min(3).max(120).messages({
		'string.empty': 'Address Line 1 is required.',
	}),
	landmark: joi.string().trim().required().min(2).max(120).messages({
