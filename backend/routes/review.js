...
router.get('/:id', async  (req, res) => {
	try {
		...
		res.json({success: 'review found!', review});
	} catch (e) {
		res.status(500).json({error: 'Internal server error'});
	}
})
...
router.post('/', isLoggedIn, isRoleRider, async (req, res) => {
	try {
		...
		property.save();
		return res.json({success: 'review saved successfully'});
	} catch (e) {
		res.status(500).json({error: 'Internal server error'});
	}
})
...
router.delete('/:id', isLoggedIn, isRoleAdminOrRider, async (req, res) => {
	try {
		...
		res.json({success: 'review deleted successfully', info});
	} catch (e) {
		res.status(500).json({error: 'Internal server error'});
	}
})
